...
* [EkLine GitHub action](#ekline-github-action)
  * [Input](#input)
  * [Usage](#usage)
  * [New Feature: Debug Mode](#new-feature-debug-mode)
  * [Reporters](#reporters)
...
inputs:
  ...
  ignore_rule:
    ...
    default: ''
  debug:
    description: 'Enable debug mode by setting INPUT_DEBUG to true. This appends a --debug flag to the ekline_command, providing additional debugging information during its execution.'
    default: 'false'
```
...
The EkLine GitHub Action now supports a debug mode for enhanced troubleshooting and understanding of execution flow.

### Enabling Debug Mode

Set the `INPUT_DEBUG` environment variable to "true" to enable debug mode. This action appends a `--debug` flag to the `ekline_command`, allowing it to provide more detailed output.
...
- **Troubleshooting**: Use debug mode to gain deeper insights into command execution and diagnose potential issues.
- **Development**: Developers can use the debug output to understand the script's behavior and make informed adjustments.
...
It's essentially the same as `reporter: github-pr-check` except it works not only for Pull Request but also for commit.
...
github-pr-review reporter reports results to GitHub PullRequest review comments using GitHub Personal API Access Token. [GitHub Enterprise](https://enterprise.github.com/home) also supports it.
...
Note that not all reporters provide full support of filter mode due to API limitation. For example, `github-pr-review` reporter uses [GitHub Review API](https://developer.github.com/v3/pulls/reviews/) but it doesn't support posting comment outside diff (`diff_context`), so EkLine reviewer uses [Check annotation](https://developer.github.com/v3/checks/runs/) as fallback to post those comments [1].
...
- [1] Reports results which are outside diff context with Check annotation as fallback if it's running in GitHub actions instead of Review API (comments). Also, report all results to console.
...
The `--debug` flag, enabled by setting `INPUT_DEBUG` to "true", provides additional information during the execution of the `ekline_command`. This is particularly useful for:

- Understanding the command's operation in detail.
- Identifying and resolving issues during the action's execution.

## Troubleshooting

### Using the `--debug` Flag

For diagnosing issues or gaining a better understanding of the `ekline_command` execution, enable the `--debug` flag by setting `INPUT_DEBUG` to "true". This provides detailed output that can help identify and resolve problems.